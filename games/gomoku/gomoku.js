...
function aiMove() {
  ...
  for (let [x, y] of list) {
    put(x, y, WHITE);
    let val = -alphaBeta(depth - 1, -1e9, 1e9, BLACK);
    undoMove(x, y);
...
// ===== Alpha-Beta 搜索 (改进版) =====
function alphaBeta(depth, a, b, player) {
  ...
  for (let [x, y] of list) {
    put(x, y, player);
    let val = -alphaBeta(depth - 1, -b, -a, 3 - player);
    undoMove(x, y);
...
function moves() {
  ...
  return list.slice(0, 40).map(i => [i[1], i[2]]);
}
...
// ===== 评分 (强化版 - 活三、冲四、活四识别) =====
function evaluate() {
  return score(WHITE) * 1.3 - score(BLACK);
}

function scorePos(x, y) {
  board[y][x] = WHITE;
  let s = score(WHITE) * 1.3 + score(BLACK);
  board[y][x] = 0;
  return s;
}
  ...
  for (let y = 0; y < SIZE; y++) {
    for (let x = 0; x < SIZE; x++) {
      ...
      for (let [dx, dy] of dirs) {
        ...
        else if (cnt === 4 && open === 1) s += 100000; // 冲四
        else if (cnt === 4) s += 50000;
        else if (cnt === 3 && open === 2) s += 10000; // 活三
        else if (cnt === 3 && open === 1) s += 5000; // 眠三
